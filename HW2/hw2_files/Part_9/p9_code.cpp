...
// compile command
// cl /EHsc hw2_files\Part_9\p9_code.cpp /I Y:\opencv\build\include /link /LIBPATH:Y:\opencv\build\x64\vc15\lib opencv_world455.lib
...
{
	// Read the image file
	Mat originalImage = imread("hw2_files/p9_search.png");
	...
	Mat final_image = rgb[0];
	merge(rgb, 3, final_image);
	...
	imwrite("hw2_files/Part_9/p9_search_final.png", final_image);
...
void OperateOnRow(int top_row_index, int bottom_row_index, Mat& binaryImage, Mat& binaryImageInverted, Mat& r, Mat& g, Mat& b, int cropped_rows, int cropped_columns) {
	...
	*/
	...
	int cell_width_x1[] = { 67, 130, 193, 256, 319, 382, 445, 508, 571, 633, 696, 759, 822, 885, 948 };
	...
	*/
	...
	for (i = 0; i < symbolCountWithoutNegatives; i++) {
		if (four_pattern) {

			for (k = top_row_index; k < bottom_row_index; k++) {
				for (l = cell_width_x0[i]; l < cell_width_x1[i]; l++) {
					if (binaryImageInverted.at<uint8_t>(k, l) != 0) {
						if (i < 4) {
							r.at<uint8_t>(k, l) = 0;
							g.at<uint8_t>(k, l) = 0;
							b.at<uint8_t>(k, l) = 255;
						}
						else {
							r.at<uint8_t>(k, l) = 255;
							g.at<uint8_t>(k, l) = 0;
							b.at<uint8_t>(k, l) = 0;
						}
					}
