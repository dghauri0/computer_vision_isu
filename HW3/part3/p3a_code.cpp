...
// compile command
// cl /EHsc part3\p3a_code.cpp /I Y:\CodeLib\opencv\build\include /link /LIBPATH:Y:\CodeLib\opencv\build\x64\vc15\lib opencv_world455.lib
...
int main(int argc, char* argv[]) {
	...
	VideoWriter output_cap("part3/p3a_video2_final.avi",
			...
			if (frame.at<Vec3b>(y_coord, x_coord)[0] >= 0 && frame.at<Vec3b>(y_coord, x_coord)[0] <= 20) {
				if (frame.at<Vec3b>(y_coord, x_coord)[1] >= 0 && frame.at<Vec3b>(y_coord, x_coord)[1] <= 20) {
					if (frame.at<Vec3b>(y_coord, x_coord)[2] >= 200 && frame.at<Vec3b>(y_coord, x_coord)[2] <= 255) {
						...
						// color circle blue (as it has been identified as red with some delta)
						//printf("Wow a red circle");
						circle(frame, center, radius + 2, Scalar(255, 0, 0), -1, FILLED);
					}
				}
			}
